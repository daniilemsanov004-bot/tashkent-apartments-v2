...
import { refreshMarketStatsIfStale, loadMarketStatsMap, loadMarketStatsManualMap, evaluateDeal } from './marketStats.js';
...
async function processSource(fetchList, fetchDetails, sourceName, dealType, fetchSellerCount = null, propertyType = 'apartment', marketStatsMap = null, manualStatsMap = null) {
  ...
  for (const item of items) {
    ...
    const { belowMarket, belowMarketPct, sampleSize, source: marketStatsSource } = marketStatsMap
      ? evaluateDeal(
          marketStatsMap,
          {
            propertyType,
      ...
      : { belowMarket: false, belowMarketPct: null, sampleSize: null, source: null };
    ...
    const listing = {
      ...
      // 'computed' — посчитано по реальным объявлениям (см. evaluateDeal
      // в marketStats.js); 'manual' — реальных данных для этой группы
      // ещё не хватило, взят ручной ориентир из market_stats_manual
      // (см. supabase/15_market_stats_manual.sql); null — вообще нет
      // оценки. Хранится отдельно от below_market, чтобы не менять
      // смысл уже существующих полей задним числом.
      market_stats_source: marketStatsSource,
...
async function main() {
  ...
  // Ручной ориентир (см. supabase/15_market_stats_manual.sql) — грузится
  // один раз за прогон, как и marketStatsMap. Если таблицы ещё нет
  // (миграция не накатана), getAllMarketStatsManual() вернёт пустой
  // список и evaluateDeal просто не найдёт в нём совпадений — ничего
  // не сломается для тех, кто ещё не применил 15_market_stats_manual.sql.
  const manualStatsMap = await loadMarketStatsManualMap();

  // Продажа — в приоритете, проверяем её первой в каждом цикле
  for (const propertyType of OLX_PROPERTY_TYPES) {
    await processSource(fetchOlxListings, fetchOlxDetails, 'olx', 'sale', fetchOlxSellerListingsCount, propertyType, marketStatsMap, manualStatsMap);
    await olxDelay();
  }
  for (const propertyType of UYBOR_PROPERTY_TYPES) {
    await processSource(fetchUyborListings, fetchUyborDetails, 'uybor', 'sale', null, propertyType, marketStatsMap, manualStatsMap);
  }
  for (const propertyType of REALTING_PROPERTY_TYPES) {
    await processSource(fetchRealtingListings, fetchRealtingDetails, 'realting', 'sale', null, propertyType, marketStatsMap, manualStatsMap);
  }
  ...
  for (const propertyType of JOYMEE_PROPERTY_TYPES) {
    await processSource(fetchJoymeeListings, fetchJoymeeDetails, 'joymee', 'sale', null, propertyType, marketStatsMap, manualStatsMap);
  }

  for (const propertyType of OLX_PROPERTY_TYPES) {
    await processSource(fetchOlxListings, fetchOlxDetails, 'olx', 'rent', fetchOlxSellerListingsCount, propertyType, marketStatsMap, manualStatsMap);
    await olxDelay();
  }
  for (const propertyType of UYBOR_PROPERTY_TYPES) {
    await processSource(fetchUyborListings, fetchUyborDetails, 'uybor', 'rent', null, propertyType, marketStatsMap, manualStatsMap);
  }
  for (const propertyType of REALTING_PROPERTY_TYPES) {
    await processSource(fetchRealtingListings, fetchRealtingDetails, 'realting', 'rent', null, propertyType, marketStatsMap, manualStatsMap);
  }
  ...
  // scrapers/joymee.js).
  for (const propertyType of JOYMEE_PROPERTY_TYPES) {
    await processSource(fetchJoymeeListings, fetchJoymeeDetails, 'joymee', 'rent', null, propertyType, marketStatsMap, manualStatsMap);
  }
