...
// Пересчитываем не на каждом прогоне (каждые 15 минут это лишняя
// нагрузка на базу без всякой пользы — медианы за 15 минут не успеют
// заметно измениться), а раз в сутки с запасом.
const STATS_MAX_AGE_MS = 20 * 60 * 60 * 1000; // 20 часов
...
/**
 * Пересчитывает market_stats, если данные устарели (см.
 * STATS_MAX_AGE_MS) или таблица ещё пустая. Вызывать один раз в
 * начале прогона (см. run.js main()), ДО обработки объявлений — сами
 * объявления этого же прогона используют уже свежую статистику.
 */
export async function refreshMarketStatsIfStale() {
  const freshness = await getMarketStatsFreshness();
  if (freshness && Date.now() - new Date(freshness).getTime() < STATS_MAX_AGE_MS) {
    console.log('Рыночная статистика ещё свежая — пропускаю пересчёт.');
    return;
  }
